...
export function ConfigsPage() {
  ...
  return (
    <div className="page-card">
      <PageTitle title="系统参数" description="维护平台级配置项，便于后续模块统一读取。" extra={<Space><Permission permission="system:config:create"><Button type="primary" icon={<PlusOutlined />} onClick={() => { setEditingItem(null); form.setFieldsValue({ status: 1, value_type: 'string' }); setOpen(true) }}>新建参数</Button></Permission><Permission permission="system:config:view"><Button onClick={async () => { const response = await exportConfigsApi(); const blob = new Blob([response.data], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' }); const url = window.URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = 'configs.xlsx'; a.click(); window.URL.revokeObjectURL(url) }}>导出参数</Button></Permission><Permission permission="system:config:create"><Upload showUploadList={false} accept=".xlsx" customRequest={async ({ file, onSuccess, onError }) => { try { const formData = new FormData(); formData.append('file', file as Blob); await importConfigsApi(formData); message.success('导入成功'); await loadData(); onSuccess?.({}, new XMLHttpRequest()) } catch (error) { onError?.(error as Error) } }}><Button>导入参数</Button></Upload></Permission><Button icon={<ReloadOutlined />} onClick={() => void loadData()}>刷新</Button></Space>} />
      <Table<ConfigItem>
        rowKey="id"
        loading={loading}
        dataSource={data}
